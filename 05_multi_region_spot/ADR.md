...
- スポットインスタンスは中断が不可避であり、単一リージョンに依存すると再起動不能になるリスクが高い
- マネージド DB に比べて運用負荷は増えるが、大幅なコスト削減が見込める
- ライブマイグレーションや多重化構成は、スポットの特性（強制終了・短い猶予時間）と相性が悪いため採用しない
- 書き込み可用性よりも、データ永続性と復旧可能性を優先するユースケースを想定
...
```
Q4 なぜ、起動させるデータベースが単数なのか？
A4 リージョン間でデータベース間でデータ整合性を保つのが難しかったから。また、リージョン内で分散を行っても参照するデータセンターが同じで、同じ需要状況が適応されるため一気に中断される可能性が高い。
一方で、AZ間分散は十分に検証の余地がある。
```
